...
class Maze:
	...
	def write_svg(self, filename):
		...
		with open(filename, 'w') as f:
			...
			print('<circle cx="25" cy="25" r="15" stroke="black" stroke-width="3" fill="green" />', file=f)
			print('<circle cx="575" cy="25" r="15" stroke="black" stroke-width="3" fill="blue" />', file=f)
			print('<circle cx="25" cy="375" r="15" stroke="black" stroke-width="3" fill="red" />', file=f)
			print('<circle cx="575" cy="375" r="15" stroke="black" stroke-width="3" fill="yellow" />', file=f) 
			print('</svg>', file=f)
	...
	def close_road(self): #Horma bat sortuz bidea itxi, aukera posible bakarra egon dadin
		end = 0
		k = 0
		while not end:
			if k == 4:
...
with open("galderak.csv", "w") as out:
	...
	while i < n:
		...
		if zuzena != 0:
			...
			out.write(f"Laberintoa,Ertz berdetik (I-M) abiatuta topatu gertueneko irteera.,{mazename},{erantzuna},{Oker1},{Oker2},,,Gorka Urbizu Garmendia\n")
			print(erantzuna)
			i += 1
